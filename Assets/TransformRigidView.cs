...
namespace MonobitEngine
{
    ...
    public class TransformRigidView : MonobitEngine.MonoBehaviour
    {
        ...
        [SerializeField]
        public bool m_SyncRigid = true;     // rigidbodyの同期をするか
        ...
        private Vector3 m_LastUpdateVelocity = Vector3.zero;        /**< 受信した最新の速度. */
        private Vector3 m_LastUpdateAngularVelocity = Vector3.zero;     /**< 受信した最新の角速度. */
        ...
        private void UpdateRigid()
        {
            if(m_SyncRigid)
            {
                ...
                rigid.velocity = m_LastUpdateVelocity;
                rigid.angularVelocity = m_LastUpdateAngularVelocity;
            }
        }
        ...
        public override void OnMonobitSerializeViewWrite(MonobitStream stream, MonobitMessageInfo info)
        {
            ...
            if(m_SyncRigid)
            {
                stream.Enqueue(rigidbody.velocity);
                stream.Enqueue(rigidbody.angularVelocity);
            }
        }
        ...
        public override void OnMonobitSerializeViewRead(MonobitStream stream, MonobitMessageInfo info)
        {
            ...
            if (m_SyncRigid)
            {
                m_LastUpdateVelocity = (Vector3)stream.Dequeue();
                m_LastUpdateAngularVelocity = (Vector3)stream.Dequeue();
            }
        }
    }
}
